...
class Solution:
    def mapWordWeights(self, words: List[str], weights: List[int]) -> str:
        ...
        return ''.join([ascii_lowercase[::-1][sum(d[char] for char in word)%26] for word in words])


class Solution:
    def mapWordWeights(self, words: List[str], weights: List[int]) -> str:
        return''.join(chr(122-sum(weights[ord(char)-97]for char in word)%26)for word in words)
