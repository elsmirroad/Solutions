...
class Solution:
    def minOperations(self, nums: List[int]) -> int:
        n = len(nums)
        if n < 2: return 0
        dec = sum(1 for i in range(n) if nums[i-1] > nums[i])
        inc = sum(1 for i in range(n) if nums[i-1] < nums[i])
        if dec != 1 and inc != 1: return -1
        idx = nums.index(0)
        if dec == 1: total = min(idx, (n-idx)%n+2)
        else: total = min(1 + (n-1-idx)%n, (idx+1)%n+1)
        return total
