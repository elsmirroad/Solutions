...
class Solution:
    def minimumCost(self, source: str, target: str, original: List[str], changed: List[str], cost: List[int]) -> int:
        ...
        for fr, to, cst in zip(original, changed, cost):
            graph[fr].append((cst, to))
        ...
        def dijkstra(src, tar):
            ...
            while q:
                ...
                for cstN, neighbor in graph[node]:
                    if cst + cstN < d[neighbor]:
                        d[neighbor] = cst + cstN 
                        heapq.heappush(q, (cst+cstN, neighbor))
            return d[tar]

        lens = sorted(set(len(s) for s in original))
