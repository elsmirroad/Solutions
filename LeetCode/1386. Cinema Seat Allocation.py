...
class Solution:
    def maxNumberOfFamilies(self, n: int, reservedSeats: List[List[int]]) -> int:
        check, total = defaultdict(set), 2 * n
        for r, s in reservedSeats:
            if s in (2, 3, 4, 5): check[r].add(0)
            if s in (4, 5, 6, 7): check[r].add(1)
            if s in (6, 7, 8, 9): check[r].add(2)
        for row in check: total -= 2 if len(check[row]) == 3 else 1
        return total
