...
class Solution:
    def earliestFinishTime(self, landStartTime: List[int], landDuration: List[int], waterStartTime: List[int], waterDuration: List[int]) -> int:
        land = min(k+v for k, v in zip(landStartTime, landDuration))
        water = min(k+v for k, v in zip(waterStartTime, waterDuration))
        wl = min(max(water, start) + duration for start, duration in zip(landStartTime, landDuration))
        lw = min(max(land, start) + duration for start, duration in zip(waterStartTime, waterDuration))
        return min(wl, lw)
