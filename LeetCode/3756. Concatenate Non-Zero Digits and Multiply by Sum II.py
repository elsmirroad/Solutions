...
class Solution:
    def sumAndMultiply(self, s: str, queries: List[List[int]]) -> List[int]:
        ...
        ps = [0] * n; ps[0] = d
        pc = [0] * n; pc[0] = 1 if d != 0 else 0
        px = [0] * n; px[0] = d if d != 0 else 0
        ...
        for l, r in queries:
            ...
            nonZero = pc[r] - (pc[l-1] if l-1 >= 0 else 0)
            x = (px[r] - (px[l-1] * pow(10, nonZero, mod) if l-1 >= 0 else 0)) % mod 
            result.append((x * sm) % mod)
        return result
