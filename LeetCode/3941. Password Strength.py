...
class Solution:
    def passwordStrength(self, password: str) -> int:
        ...
        au = set(string.ascii_uppercase)
        ad = set(string.digits)
        ac = set('!@#$')
        for char in set(password):
            if char in al: total += 1
            elif char in au: total += 2
            elif char in ad: total += 3
            elif char in ac: total += 5
        return total
...
class Solution:
    def passwordStrength(self, password: str) -> int:
        total = 0
        for char in set(password):
            if char.islower(): total += 1
            elif char.isupper(): total += 2
            elif char.isdigit(): total += 3
            elif char in "!@#$": total += 5
        return total
