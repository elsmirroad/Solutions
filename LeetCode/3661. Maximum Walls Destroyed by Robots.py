...
class Solution:
    def maxWalls(self, robots: List[int], distance: List[int], walls: List[int]) -> int:
        ...
        if n == 1:
            firstRight = arr[0][0] + arr[0][1]
        else:
            firstRight = min(arr[0][0] + arr[0][1], arr[1][0] - 1)

        dp[0][1] = count_walls(arr[0][0], firstRight)

        for i in range(1, n):
            pos, dist = arr[i]

            rightEnd = min(pos + dist, arr[i + 1][0] - 1)
            rightWalls = count_walls(pos, rightEnd)

            dp[i][1] = max(dp[i-1][0], dp[i-1][1]) + rightWalls

            leftStart = max(pos - dist, arr[i-1][0] + 1)
            leftWalls = count_walls(leftStart, pos)

            dp[i][0] = dp[i-1][0] + leftWalls

            prevRight = min(arr[i-1][0] + arr[i-1][1], pos - 1)

            overlapStart = leftStart
            overlapEnd = min(prevRight, pos - 1)
            overlapWalls = count_walls(overlapStart, overlapEnd)
