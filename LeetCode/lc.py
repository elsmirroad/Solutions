...
class TreeNode:
    ...
    def __repr__(self):
        return (
            f := lambda x: x
            and f"TreeNode{{val: {x.val}, left: {f(x.left)}, right: {f(x.right)}}}"
            or "None"
        )(self)
...
class ListNode:
    ...
    def __repr__(self):
        ...
        return (
            f := lambda x: x
            and f"ListNode{{val: {x.val}, next: {f(x.next)}}}"
            or "None"
        )(self)
...
def test(
    ...
    def vcast(func, args, init=None):
        ...
        if init:
            ...
            args = [
                vc(init, init.__code__.co_varnames[i], x) for i, x in enumerate(args)
            ]
        else:
            args = [
                vc(func, func.__code__.co_varnames[i + d], x)
                for i, x in enumerate(args[:argc])
            ]
    ...
    def print_res(passed, res, expected, *args):
        g = lambda t: str(
            t.serialize(t) if type(t) is ListNode or type(t) is TreeNode else t
        )
        c = lambda c, t, w=50: "\x1b[{1}m{2}\x1b[0m".format(
            s := g(t), 30 + c, s[:w] + "..." if len(s) >= w else s
        )
        e = 2 if passed else 1
        print(
            "%s args %s result %s expected %s"
            % (
                c(e, "PASSED" if passed else "FAILED"),
                c(e, args),
                c(e, res),
                c(e, expected),
            )
    ...
    if not check:
        ...
        def check(res, expected, *args):
            ...
            if inplace:
                ...
                if expected == "[]":
                    expected = None  # 114.flatten-binary-tree-to-linked-list.py
    ...
    custom_class_tests = (
        classname is not None and "Launcher" not in str(classname) and custom != False
    )
    ...
    if not custom_class_tests:
        for s in text.splitlines():
            if s.startswith("Input:"):
                ...
                s == ""
                or any(
                    s.startswith(t) for t in ("Exampl", "Explan", "Operat", "Note", "#")
                )
            ):
    ...
    for cname in cnames:
        ...
        for t in tests:
            ...
            func = getattr(
                cname(), [*filter(lambda s: not s.startswith("__"), dir(cname))][-1]
            )
            ...
            if (
                type(ok) is tuple
            ):  # see linked-list-cycle-ii (substitutes res with custom res)
                ok, res = ok[:2]
...
def test_file(filename):
    with open(os.devnull, "wb") as devnull:
        process = subprocess.Popen(
            ["py", filename], stderr=subprocess.PIPE, stdout=subprocess.PIPE
        )
