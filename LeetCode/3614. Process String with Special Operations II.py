...
class Solution:
    def processStr(self, s: str, k: int, length = 0) -> str:
        for char in s:
            if char == "*": length -= 1 if length > 0 else 0
            elif char == "#": length *= 2
            elif char == "%": continue
            else: length += 1
        if not k < length: return "."
        for char in reversed(s):
            if char == "*": length += 1
            elif char == "#":
                length //= 2
                if k >= length: k -= length
            elif char == "%": k = length - 1 - k
            elif k == length - 1: return char
            else: length -= 1
