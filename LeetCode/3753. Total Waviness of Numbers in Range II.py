...
class Solution:
    ...
    def waveCount(self, num):
        if num < 100: return 0
        return sum(self.countWays(num, p) for p in self.nums)
    ...
    def countWays(self, num, pattern):
        ...
        for i in range(n-2):
            left = int(s[:i] or 0)
            mid = int(s[i:i+3])
            right = int(s[i+3:] or 0)
            winds = 10 ** (n-i-3)
            ways = 0
            if mid > pattern: ways = left - t + 1
            elif mid == pattern: ways = max(0, left-t); total += right + 1
            else: ways = max(0, left-t)
            total += ways * winds
        return total
